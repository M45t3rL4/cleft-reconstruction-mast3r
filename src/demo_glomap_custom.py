...
from thirdparty.Mast3r.mast3r.colmap.mapping import kapture_import_image_folder_or_list, run_mast3r_matching, pycolmap_run_mapper, glomap_run_mapper
from thirdparty.Mast3r.mast3r.demo import set_scenegraph_options
from thirdparty.Mast3r.mast3r.retrieval.processor import Retriever
from thirdparty.Mast3r.mast3r.image_pairs import make_pairs

import mast3r.utils.path_to_dust3r  # noqa
from thirdparty.Mast3r.dust3r.dust3r.utils.image import load_images
from thirdparty.Mast3r.dust3r.dust3r.viz import add_scene_cam, CAM_COLORS, OPENGL
from thirdparty.Mast3r.dust3r.demo import get_args_parser as dust3r_get_args_parser
from thirdparty.hloc.hloc import (
    extract_features,
    match_features,
    reconstruction,
    visualization,
...
def get_args_parser():
    ...
    parser.add_argument('--model_name', default='MASt3R_ViTLarge_BaseDecoder_512_catmlpdpt_metric', type=str, help="retrieval_model to be loaded")
...
def make_netvlad_pairs(output_path, image_list, imgs, num_matched= 5):

    out_dir = output_path +"/netv_pairs"
    os.makedirs(out_dir, exist_ok=True)
    sfm_pairs = out_dir + "/pairs-netvlad.txt"
    retrieval_conf = extract_features.confs["netvlad"]
    retrieval_path = extract_features.main(retrieval_conf, Path(image_list[0]).parent, out_dir, image_list=image_list)
    ...
    with open(sfm_pairs, 'r') as file:
        for line in file:
            ...
            for i in range(len(image_list)):
                if image_list[i].endswith(img_names[0]):
                    idx1 = i
                if image_list[i].endswith(img_names[1]):
                    idx2 = i
                if idx1 is not None and idx2 is not None:
                    break
            idx_list.append((imgs[idx1], imgs[idx2]))


    #transform .txt to numpy. array
    return idx_list
def get_reconstructed_scene(glomap_bin, outdir, gradio_delete_cache, model, retrieval_model, device, silent, image_size,
                            ...
                            win_cyclic, refid, shared_intrinsics, dense_matching=False, conf_thr=1.001, num_matched=5, skip_geometric_verification=False, options=None, **kw):
    """
    ...
    if 'netvlad' in scenegraph_type:
        pairs = make_netvlad_pairs(output_path=outdir, image_list=filelist,imgs=imgs,num_matched=num_matched)
    else:
    ...
    pycolmap_run_mapper(colmap_db_path, reconstruction_path, root_path)#TODO can add options
    ...
    ouput_recon = pycolmap.Reconstruction(reconstruction_path + "/0")
...
def main_demo(glomap_bin, tmpdirname, model, retrieval_model, device, image_size, server_name, server_port,
              silent=False, share=False, gradio_delete_cache=False, skip_geometric_verification=False):
    if not silent:
        print('Outputing stuff in', tmpdirname)

    recon_fun = functools.partial(get_reconstructed_scene, glomap_bin, tmpdirname, gradio_delete_cache, model,
                                  retrieval_model, device, silent, image_size,skip_geometric_verification)
